...
var RESPONSES = [
		[ undefined, [
			...
			/(гея )?(ищ[ую]|хочу) ([мжпд] |[мжпд][)]*$|би |парня|девушку|девочку|лезби|девчонку|женщину|даму|горячую|полненькую|послушн|взросл|зрелую|сочную|суч?ку|раба|гея|господина|лесби|подругу|строгую|прекрасн|похотлив|собеседницу|нижнюю|милого|милую|сладенькую|плохую|опытную|казашек|пассива|приятную|хозяина|отбит)/i,
			...
			/^покажи/i, /пока(жи|жите|зать)? *(себя|свою|свои|ваши|лицо|ножки|ноги|груд[ьи]|трус)/i, / фото([ .?)☺️!$]|чк|к)/i, /фотк/i, /фото$/i,
			...
			/(возможно|Если) ты девушка/i,

			/^пр(иф|еф|ев)?$/i, /^пока[.]?$/i,
			/как (зовут|звать) тебя[?)]*$/i, /как (тебя|вас) (зовут|звать)[,?) :]*$/i, /как (зовут|звать)[? )]*$/i, /^имя.?$/i, /^как *(тво.)? *имя[?]?$/i,
			/Привет как зовут[?]/i,
			...
			/^(ты )?откуда/i,
			/^(привет[., ])?(я )?(макс|Макс|егор|влад|боря|вадим|иван|дима|Олег|денис|саня|сергей|павел|евгений|дмитрий|ваня|вова|стас|игорь|дима|витя|илья|алексей|артем|валентин|георгий|арсений|миша|андрей|рома|вася|коля|николай|александр|кирилл|гоша|дани+л|Никита|костя|сергей|паша|михаил|антон|виталий)/i,
				/Привет, я Макс/i, /Привет , я Егор/i,
			/^(привет[., ])?(я )?(лиза|алина|наст[яь]|маша|наташа|Кристина|юля|вика|яна|лера|мария|арина|оля)/i,
			/^(спб|екб|мск|питер|москва|новосиб|киев|магнитогорск|казахстан|воркута|казань)/i,
			...
			/Расскажи что-нибудь интересное/i, /не молчи/i, /(писос )?скучно$/i,

			/ точк[аи][)?]*$/i, /^то+чк[аи]/i, /точечка/i, /^(все *(настолько)?|как)? *серь.зно[)]?$/i, /^загадочно так/, /грозно[?]*/i,
			/такой серьёзный[?. ]*$/i,
			...
			/шени деда моутхна кунем мама/i, / ис реал/i, /Джумин Хан/i, /хоп хэй лалалей/i, /КАК ПОГОД.?А *, *БОБР/i, /как вы планируете захватить мир/i,
			/Не уходи смиренно, в сумрак вечной тьмы/i, /ищу енота в гусси/i,
			/ты не шаришь/i
		]	],
		...
		[ 'Привет.', [
			...
			/^салют/i,
			/^ал+о$/i,
			/^(добрый веч.р|вечер добрый)[. ?)!:^]*$/i, /^доброго вечера/i, /^добро(е|го) врем(я|ени) суток[. ?]*$/i,
			/^з?дра+сь*т[ьеи][ ).?^]*/i, /^здравствуйте.?$/i, /^здрав?ствуй[?)cс:.]*$/i, /^[з]?д[ао]ров(а)?[.]?$/i, /^здаровки/i
		]	],
		[ 'Шевелится.', [
			/^как оно[?) ]*$/i
		]	],
