...
var RESPONSES = [
		[ undefined, [
			...
			/(гея )?(ищ[ую]|хочу) ([мжпд] |[мжпд][)]*$|би |парня|девушку|девочку|лезби|девчонку|женщину|даму|горячую|полненькую|послушн|взросл|зрелую|сочную|суч?ку|раба|гея|господина|лесби|подругу|строгую|прекрасн|похотлив|собеседницу|нижнюю|милого|милую|сладенькую|плохую|опытную|казашек|пассива)/i,

			/^(привет[,. !)]*)?(я|ты|вы|[0-9]+)? *([мждпM][,. )?0-9!]*$|[мждпMш][,. !)?0-9]+|[мпжд] *(или|,|\/|\\)? *[мпжд]([,. )?]|[,. )?]?$)|би[,. )]?$|парень|девушка|мальчик|девочка|гей|лесби|лизби|транс)/i,
			...
			/Я брюнет 184 рост, зелёные глаза, телосложение спортивное/i,
			...
			/^(ты )?откуда/i,
			/^(привет[., ])?(я )?(макс|егор|влад|боря|вадим|иван|дима|Олег|денис|саня|сергей|павел|евгений|дмитрий|ваня|вова|стас|игорь|дима|витя|илья|алексей|артем|валентин|георгий|арсений|миша|андрей|рома|вася|коля|николай|александр|кирилл|гоша|дани+л|никита|костя|сергей|паша|михаил)/i,
			/^(привет[., ])?(я )?(лиза|алина|наст[яь]|маша|наташа|Кристина|юля|вика|яна)/i,
			/^(спб|екб|мск|питер|москва|новосиб|киев|магнитогорск|казахстан|воркута)/i,

			/https?:/i,
			/^мур( мур)?[:3 ]*$/i, /^мяу$/i, /^гоу? /i, /^(лол|кек)/i, /кис.кис.кис/i, /^зиг/i, /^воу$/i,
		...
		[ 'Привет.', [
			/^[.]*(пр+и+в+[еэі]+[тиь].?|прива?)[ ^_?;:.30)!сc]*(собеседник)?$/i, /^привте$/i, /^приветики?[ .:з)]?$/i, /^привки$/i,
			/^х[аеэ]+й.?$/i,
		...
		[ 'Хорошо. И у тебя.', [
			/^(ну )?как (дела+|жизнь|жиза|настрой|настроени[ея])( молодая)?[?) \\]*$/i
		]	],
		[ 'Спокойно. А у тебя?', [
		...
		[ 'Собеседников ищу, как и ты.', [
			/^чем занимаешься[?) ]*$/i, /^ч(то|е|его) делаешь[?) ]*$/i
		] ]
	]
