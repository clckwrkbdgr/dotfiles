...
var RESPONSES = [
		[ undefined, [
			...
			/(гея )?(ищ[ую]|хочу) (себе *)?([мжпд] |[мжпд][)]*$|би |парня|девушку|девочку|лезби|девчонку|женщину|даму|горячую|полненькую|послушн|взросл|зрелую|сочную|суч?ку|раба|гея|господина|лесби|подру(гу|жку)|строгую|прекрасн|похотлив|собеседницу|нижнюю|милого|милую|сладенькую|плохую|опытную|казашек|пассива|приятную|хозяина|отбит|пышную|полную)/i,

			/^(привет[,. !)]*)?(я|ты|вы|[0-9]+)? *([мждпM][,. )?0-9!]*$|м-ж|[мждпMш][,. !)?0-9]+|[мпжд] *(или|,|\/|\\)? *[мпжд]([,. )?]|[,. )?]?$)|(м ?)?би[,. )]?$|паре[нг]ь|девушка|мальчик|девочка|гей|лесби|лизби|транс)/i,

			/пошл(ячк|еньк|ую|ого|ой|ая|ый|ых|ост)/i,
			/дроч(ил|им|у|ер|ить|ишь)/i, /кончу /i, /помоги кончить/i, /отли(зал|жу)/i, /лизать/i, /кончил на/i, /возбуди/i, /шалить/i, /шалим/i, /дрочу/i, /мастурби/i,
			/пошалю/i, /разденусь/i, /^познакомлюсь/i, /трахни /i,
			/[, ]вирт/i, /^вирт[ )]/i, /фотовирт/i, /секс/i, /скайп/i, /виpтик/i, /^вирт[?)]*$/i, /бдсм/i, /спермы/i,
			/потраха/i, /трахал/i, / куни/i, /пид.р/i, /оттраха/i, /инцест/i, /нимфоман/i, /оргазм/i, / выебу /i, / ублаж/i,
			/грудью/i, /прелести/i, /сис(и|ями|ьки|ечк|ек|ька)/i, /тит(ечки|ьки)/i, /письк/i, /скинь груд/i,
			/^покажи/i, /(пока(жи|жет|жите|зать)?|сфотк?ай|кинь) *(себя|свою|свои|ваши|лицо|ножки|ноги|груд[ьи]|трус)/i, / фото([ .?)☺️!$]|чк|к)/i, /фотк/i, /фото$/i,
			/ножки покажи/i,
			/покажи свои/i, /порадуй .*(ножками|фото)/i, /женск[^ ]+ тел/i, /покажи грудь/i, /порадуешь.*парня/i,
			/страпон/i, /интим/i, /минет/i, /фут *фетиш/i, /шлю[хш]/i, /извращен/i, /раб(ын|ом)/i, /госпож/i, /разврат/i, / член/i, /^член$/i, /пенис/i, /страстн/i,
			/трусики/i, /нудист/i, /курвочку/i, /лифчик/i, /Дрочер/i,
			/^[0-9]+ *см/i,
			/^девушк.*(есть|остались)[?]*$/i, /есть девушки/i,
			/^сук[аи]/i, /сучк[ауи]/i, /иди(те)? на *хуй/i, /^хуй/i, /соси хуй/i,
			...
			/как (зовут|звать) тебя[?)]*$/i, /как (теб[яе]|вас) (з.вут|звать)[,?) :]*$/i, /как (зовут|звать)[? )]*$/i, /^имя.?$/i, /^как *(тво.)? *имя[?]?$/i,
			/Привет как зовут[?]/i,
			...
			/^(ты )?откуда/i,
			/^(привет([., !]|\)))?(я )?(макс|Макс|егор|влад|боря|вадим|иван|дима|Олег|денис|саня|сергей|павел|евгений|дмитрий|ваня|вова|стас|игорь|дима|витя|илья|алексей|артем|валентин|георгий|арсений|миша|андрей|рома|вася|коля|николай|александр|кирилл|гоша|дани+л|Никита|костя|сергей|паша|михаил|антон|виталий|леша|саша)/i,
				/Привет, я Макс/i, /Привет , я Егор/i, /Привет, я Олег/i,
			/^(привет[., ])?(я )?(лиза|алина|наст[яь]|маша|наташа|Кристина|юля|вика|яна|лера|мария|арина|оля|елена)/i,
			/^(спб|екб|мск|питер|москва|новосиб|киев|магнитогорск|казахстан|воркута|казань|челябинск|оренбург)/i,
			...
			/Расскажи что-нибудь интересное/i, /не молчи/i, /(писос )?скучно$/i, /молчишь[?]*$/i, /Ну давай рассказывай/i, /^скука$/i,

			/ точк[аи][)?]*$/i, /^то+чк[аи]/i, /точечка/i, /^(все *(настолько)?|как|шо так|(чёт )?так)? *(серь.зно|грубо)[)]?$/i, /^загадочно так/, /грозно[?]*/i,
			/такой серьёзный[?. ]*$/i,
			...
			/реборн/i, /Ира жила в приюте/i, /Где потеряна была честь твоя/, /Где была потеряна честь твоя/, /Май тичер/i, /гарри и луи/i, /^Поторопись/i,
			/Скажи (мне )?(3|три) главных слова/i, /АДЕЛЬ АДЕЛЬ ДАЙ ПОЖРАТЬ СКОРЕЙ/i,
			/ты не шаришь/i
		]	],
