...
var RESPONSES = [
		[ undefined, [
			...
			/потрахаем/i, /трахал/i, / куни/i, /пид.р/i,
			/грудью/i, /прелести/i, /сис(и|ями|ьки|ечки|ек|ька)/i, /тит(ечки|ьки)/i, /письк/i,
			...
			/^(ты )?откуда/i,
			/^(привет[., ])?(я )?(макс|влад|боря|дмитрий|игорь|дима|витя|алина|наст[яь]|илья|алексей|олег|валентин|георгий|миша|андрей|маша|наташа|рома|вася|коля|александр|кирилл|гоша|дани+л|никита|костя|сергей)/i,
			...
			/ точка[)?]*$/i, /^то+чк[аи]/i, /с точкой$/i, /точечка/i, /(все *(настолько)?|как) *серь.зно[)]?$/i, /^загадочно так/, /грозно[?]*/i,
			/ставить точк./i,
			...
			/Харли Квин/i, /ТОЛЬКО ВМЕСТЕ МЫ СИЛЬНЫ/i, /дратути/i, /^дч[?]*$/i, /^девчат$/i,
			/ты не шаришь/i
		]	],
		[ undefined, [
		...
		[ 'Собеседников ищу, как и ты.', [
			/^чем занимаешься[?) ]*$/i, /^что делаешь[?) ]*$/i
		] ]
	]
