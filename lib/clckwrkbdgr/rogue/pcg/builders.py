...
class MazeBuilder(Builder):
	def _build(self):
		layout_size = Size(
				...
				self.size.width - 2 - (1 - self.size.width % 2),
				self.size.height - 2 - (1 - self.size.height % 2),
				) # For walls.
		...
		for pos in layout.size:
			if layout.cell(pos.x, pos.y):
				self.strata.set_cell(pos.x + 1, pos.y + 1, 'floor')

		floor_only = lambda pos: self.strata.cell(pos.x, pos.y) != 'wall'
		self.start_pos = pcg.pos(self.rng, self.size, floor_only)
		Log.debug("Generated player pos: {0}".format(self.start_pos))
