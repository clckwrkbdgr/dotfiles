...
class TestWorkstationEvents(unittest.TestCase):
	...
	def should_run_arbitrary_event_script(self, is_file, run_command, save_data_path):
		...
		run_command.assert_called_once_with([sys.executable, os.path.join('/path/to/share/workstation', 'myevent.py')])
	@mock.patch('clckwrkbdgr.xdg.save_data_path')
	@mock.patch('clckwrkbdgr.commands.run_command_and_collect_output')
	@mock.patch(pathlib_module + '.Path.is_file')
	...
	def should_recognize_default_events(self, is_file, run_command, save_data_path):
		...
		run_command.assert_called_once_with([sys.executable, os.path.join('/path/to/share/workstation', 'onlogin.py')])

		run_command.reset_mock()
		self.assertTrue(workstation.onlogout())
		run_command.assert_called_once_with([sys.executable, os.path.join('/path/to/share/workstation', 'onlogout.py')])

		run_command.reset_mock()
		self.assertTrue(workstation.onlock())
		run_command.assert_called_once_with([sys.executable, os.path.join('/path/to/share/workstation', 'onlock.py')])

		run_command.reset_mock()
		self.assertTrue(workstation.onunlock())
		run_command.assert_called_once_with([sys.executable, os.path.join('/path/to/share/workstation', 'onunlock.py')])
