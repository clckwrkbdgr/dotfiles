 ...
 */
clckwrkbdgr.autoidle.setup_trigger = function(name, callable, _options)
{
	_options = _options || {};
	...
	autoidle._triggers[name] = trigger;
}
...
clckwrkbdgr.autoidle.setup_action = function(name, callable, _options)
{
	...
	autoidle._actions[name] = trigger;
}
...
clckwrkbdgr.autoidle._check_rules = function(_options)
{
	...
	for(var i = 0; i < autoidle._rules.length; ++i) {
		...
		for(var j = 0; j < rule.triggers.length; ++j) {
			var trigger = autoidle._triggers[rule.triggers[j]];
			if(!trigger) {
		...
		var action = autoidle._actions[rule.action];
		if(!action) {
