...
clckwrkbdgr.autoidle._export_settings = function()
{
	...
	for(var i = 0; i < all_rules.length; ++i) {
		...
		var rule_enabled = rule_div.querySelector('#clckwrkbdgr-autoidle-enable-rule-' + i);
		rule.enabled = rule_enabled.checked;
		console.log('out', i, rule.enabled, rule_enabled.checked);
...
clckwrkbdgr.autoidle._add_new_rule_row = function(container, rule, i)
{
	...
	console.log('in', i, rule.enabled, rule_enabled.checked);
...
clckwrkbdgr.autoidle.setup_action = function(name, callable, _options)
{
	...
	trace('Registered action "' + name + '": ' + JSON.stringify(trigger));
}
