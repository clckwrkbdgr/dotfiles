...
/** Processes and sets generic attributes for element:
 * - id;
 * - class_name - can be either single name, or array of names;
 * - style;
 */
function process_generic_options(element, options)
{
	if(options['id']) {
	...
	if(options['class_name']) {
		if(Array.isArray(options['class_name'])) {
			for(var i = 0; i < options['class_name'].length; ++i) {
				element.classList.add(options['class_name'][i]);
			}
			console.log(element);
...
clckwrkbdgr.ui.create_gui_container = function(root_element, options, controls)
{
	...
	process_generic_options(container, options);
...
clckwrkbdgr.ui.create_menu_item = function(text, onclick, options)
{
	...
	process_generic_options(menu_item, options);
...
clckwrkbdgr.ui.create_info_item = function(text, options)
{
	...
	process_generic_options(info, options);
	info.innerText = text;
	return info;
};
...
clckwrkbdgr.ui.create_button = function(caption, options)
{
	...
	process_generic_options(button, options);
	if(options['onclick']) {
...
clckwrkbdgr.ui.create_link = function(href, text, options)
{
	...
	link.innerText = text;
	process_generic_options(link, options);
	return link
};
...
clckwrkbdgr.ui.create_spin = function(title, min_value, max_value, default_value, options)
{
	...
	process_generic_options(caption, {...options, ...{id:options['id'] + '-title'}});
	caption.innerText = title;
	container.appendChild(caption);

	var button_down = document.createElement('div');
	process_generic_options(button_down, {...options, ...{
		id: options['id'] + '-dec',
		class_name: options['class_down'] || options['class_name'],
		style: options['style_down'] || options['style'],
	}});
	button_down.innerText = '-';
	container.appendChild(button_down);

	var label = document.createElement('span');
	process_generic_options(label, options);
	label.innerText = default_value;
	container.appendChild(label);

	var button_up = document.createElement('div');
	process_generic_options(button_up, {...options, ...{
		id: options['id'] + '-inc',
		class_name: options['class_up'] || options['class_name'],
		style: options['style_up'] || options['style'],
	}});
	button_up.innerText = '+';
...
clckwrkbdgr.ui.create_checkbox = function(title, options)
{
	...
	process_generic_options(checkbox, options);
	if(options['name']) {
...
clckwrkbdgr.ui.create_file_input = function(title, options)
{
	...
	process_generic_options(file_input, options);
	if(options['onload']) {
